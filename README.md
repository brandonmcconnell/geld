...
Built with [WXT](https://wxt.dev) and shipped for **Chrome, Edge, Firefox and Safari** from a single codebase. This repository is a small pnpm monorepo: the extension lives in `apps/extension`, shared logic in `packages/core` and `packages/review`, the GitHub Action in `apps/action`, and the geld.sh website in `apps/site`.
...
- **Works on the PR conversation tab too**: a review digest (open findings, bot verdicts) sits under the description, and bot comments can fold into accordions. Install the [Geld Action](apps/action/examples/geld.yml) so the digest is written before anyone opens the page; without it the extension builds the same panel from the timeline. Header counts still come from the PR's raw `.diff` when there are no file diffs on the page.
...
- **GitHub account sync (optional):** "Sign in with GitHub" (top right of the popup or options) signs you in through the **Geld GitHub App** with GitHub's *device flow* — you confirm a short code on github.com. Settings are kept in a **secret gist on your own account** (`geld-settings.json`), so there is no Geld server or database and nobody else can read or change them; the token stays on the device that signed in, expires after eight hours and is renewed by the extension. Changes push within a couple of seconds and pull when the popup/options open or the browser starts. Signing in on a second browser that already has different settings asks whether to keep that device's or use the account's. Requires the App's client id with Device Flow enabled (set `WXT_GELD_APP_CLIENT_ID` in `apps/extension/.env` at build time, or paste one in the options page). Sign-ins made with the earlier OAuth App keep working but show a "Sign in again" notice until you reconnect.
