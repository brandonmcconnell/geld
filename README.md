...
- **Adjusts the header counts** (`+400 −120`, _Files changed 17_, _"18 files changed"_, compare-page summary) so they exclude tests, and prefixes them with a purple `N tests` label (always shown, even `0 tests`, so you know the numbers were checked). When tests are present, hovering shows a tooltip with three rows: excluding tests, including tests, and tests only.
