...
- **Access.** A GitHub App named Geld with permissions Metadata: read, Pull requests: read (phase 1) → write (phase 2). Users install it on the orgs/repos they want; the dashboard shows an "Install Geld on … to see private repositories" prompt when a search omits repos the app cannot see. Public repositories are readable without installation (verify early; fall back to unauthenticated REST for public repos if not). Existing gist-scoped sign-in is untouched.

## GitHub App setup (Brandon, once)

1. Create the app "Geld" under the account: callback URLs for prod and preview per the existing env rules ([apps/site/lib/auth/config.ts](apps/site/lib/auth/config.ts)), **Device flow enabled**, "Request user authorization (OAuth) during installation" on, "Expire user authorization tokens" **off** for v1 (or implement refresh in `@geld/github` — see risks).
2. Permissions: Repository → Metadata read, Pull requests read. Phase 2 bumps Pull requests to write (users re-accept).
3. Secrets: `GELD_APP_CLIENT_ID` (public; in extension via `WXT_GELD_APP_CLIENT_ID` like the existing OAuth id) and `GELD_APP_CLIENT_SECRET` (site only, Vercel env). Public link to the installation page goes in the dashboard's empty state.

## Package: `@geld/github` (new, `packages/github`)

- Octokit GraphQL + REST clients taking a token provider (extension: `storage.local`; site: server-side session cookie as in [apps/site/lib/auth/session.ts](apps/site/lib/auth/session.ts), calls proxied through route handlers so the token never reaches the browser).
...
- Bump the app to Pull requests: write; prompt re-authorization.
...
- New WXT entrypoints `entrypoints/dashboard/` (React) and the App device-flow connect state in [apps/extension/src/lib/account-service.ts](apps/extension/src/lib/account-service.ts) (separate from gist sync; token in `storage.local`).
...
- `/dashboard` and `/review/...` pages behind the existing session; route handlers `/api/github/*` that call `@geld/github` with the server-held App token; App OAuth start/callback next to the existing gist flow in [apps/site/app/auth](apps/site/app/auth). Preview deployments keep refusing sign-in as today.
- Header gets a "Dashboard" link when signed in and connected.

## Milestones (one commit each, shippable)

1. `@geld/github` package: clients, queries, query-string helpers, fixtures + tests. GitHub App created; connect flows on both homes (device + web), token storage, "Install Geld on …" empty state.
...
- Confirm a user-to-server App token can read public repos without installation; if not, fall back to unauthenticated REST for public repos in `@geld/github`.
- App token expiry: keep "expire tokens" off for v1 or implement refresh in the client; the extension's service worker must refresh without a UI.
