...
**Fixing PR-list selectors after a GitHub markup change** works the same way, without a store release: the catalog also carries the *list surfaces* (`packages/core/src/list-surfaces.ts`, `BUNDLED_LIST_SURFACES`) — per GitHub list UI, the row selector, where the `+N −M` chip anchors, where the author is written and any chip CSS. Edit the spec, push to `main`; CI publishes the catalog and installed extensions adopt it within about six hours (or on "Check now"). New *behaviour* (a new placement mode, a new kind of author source) still needs code and a release.
