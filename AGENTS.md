...
- **Stack:** Next.js 16 App Router with `cacheComponents: true`, Tailwind CSS 4, shadcn components generated on **Base UI** (`components/ui/`), the `cn` package for class merging, Geist via the `geist` package, `lucide-react` icons. Everything is prerendered; the only cached-async piece is the latest-release lookup (`lib/release.ts`, `use cache`, revalidated hourly, falls back to the Releases page when there is no release yet). Light/dark follows `prefers-color-scheme` by default; the header's theme menu (`components/theme-menu.tsx`, `lib/theme.ts`) can pin a scheme via `data-theme` on `<html>`, persisted in localStorage and applied by an inline script before paint. Colour tokens use `light-dark()` keyed on `color-scheme`, and the `dark:` variant covers both cases (globals.css). The white brand SVGs are shown in dark mode by toggling two `<img>`s with that variant (`components/logo.tsx` references `assets/brand/` via `new URL(..., import.meta.url)`); the SVG favicon can only follow the OS.
