...
- **No server-side state.** Settings sync works from the extension alone (GitHub device flow + secret gist), so the site has no auth endpoint and no API routes; keep it that way unless a feature truly needs one.
